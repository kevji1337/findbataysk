...
async def _send_referral_details(
    ...
    await callback.message.answer(
        text=(
            ...
            f"{referral_link.invite_link}\n\n"
            f"📊 Переходов по ссылке: <b>{referral_link.referral_count}</b>\n"
...
async def referral_program(callback: types.CallbackQuery, bot: Bot) -> None:
    ...
    if existing_link:
        await _send_referral_details(callback, existing_link)
        try:
    ...
    try:
        ...
        saved_link, created_now = await ReferralRepository.create_or_get_by_user_id(
            user_id=user.id,
        ...
        if not created_now:
            try:
                ...
                logger.warning(
                    "Не удалось отозвать лишнюю invite link для пользователя {}: {}",
                    callback.from_user.id,
                    revoke_error,
                )

            await _send_referral_details(callback, saved_link)
        else:
            await notify_admin_new_referral_link(
                bot=bot,
                user_id=callback.from_user.id,
                username=callback.from_user.username,
                invite_link=saved_link.invite_link,
            )

            await callback.message.answer(
                text=(
                    f"🎉 <b>Ваша персональная реферальная ссылка создана!</b>\n\n"
                    f"🔗 {saved_link.invite_link}\n\n"
...
async def claim_referral_gift(callback: types.CallbackQuery, bot: Bot) -> None:
    ...
    if not delivered_to_admin:
        await ReferralRepository.release_claimed_gifts(referral.id, claimed)
